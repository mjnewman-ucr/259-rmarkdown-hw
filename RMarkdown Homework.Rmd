...
author: "**Madeline Newman (but actually John Franchak)**"
date: "**3/3/2022**"
output: 
  html_document:
    toc: true
...
```{r warning = FALSE, message = FALSE}
ds %>% group_by(age_group) %>% 
  summarize(across(precision, list(M = mean, MIN = min, MAX = max))) %>% kable()
```

```{r warning = FALSE, message = FALSE}
ds %>% ggplot(aes(x = age_group, y = precision)) + geom_boxplot() + 
